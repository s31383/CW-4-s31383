...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnMinSalaryInDept30()
    {
        ...
        decimal? minSalary = emps.Where(emp => emp.DeptNo == 30)
            .Min(emp => emp.Sal);
    ...
    public void ShouldReturnEmployeesWithManagers()
    {
        ...
        var withMgr = emps.Where(emp => emp.Mgr is not null); 
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, emp => emp.DeptNo, dept => dept.DeptNo,
            ...
            .SelectMany(gr => grades, (record, salgrade) => new {record.EName,record.DName,salgrade.Grade,salgrade.Losal,salgrade.Hisal,record.Sal})
            .Where(record => record.Sal < record.Hisal && record.Sal > record.Losal)
            .Select(record => new {record.EName,record.Grade,record.DName});
