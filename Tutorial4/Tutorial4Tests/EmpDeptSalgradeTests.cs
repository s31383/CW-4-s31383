...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(
            emp => depts.Where(dept => dept.Loc == "CHICAGO")
                .Select(dept => dept.DeptNo)
                .ToList().Contains(emp.DeptNo)).ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(gr => grades,
                (emp, salgrade) => new { emp.EName, salgrade.Grade, emp.Sal, salgrade.Hisal, salgrade.Losal })
            .Where(record => record.Sal < record.Hisal && record.Sal > record.Losal)
            .Select(record => new { record.EName, record.Grade });
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(emp => emp.Sal >
                                       emps.Where(e => e.DeptNo == emp.DeptNo)
                                           .Average(e => e.Sal))
            .Select(e => e.EName);
